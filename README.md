...
```bash
cd backend
cp .env.example .env          # edit model paths if needed
pip install -r requirements.txt
pip install customtkinter sounddevice soundfile   # for speech runner
pip install mediapipe opencv-python               # for eye runner
python run.py
```
